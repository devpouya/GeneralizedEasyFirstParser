...
def get_oracle_actions(fpath,transition,is_agenda=False):
    if not is_agenda:
        ...
        fname_train = '%s/easy-first-mh4_actions_%s.json' % (fpath, 'train')
        fname_dev = '%s/easy-first-mh4_actions_%s.json' % (fpath, 'dev')
        fname_test = '%s/easy-first-mh4_actions_%s.json' % (fpath, 'test')
    return (fname_train, fname_dev, fname_test)
