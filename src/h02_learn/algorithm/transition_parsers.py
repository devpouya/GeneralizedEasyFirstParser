...
class ShiftReduceParser():
    ...
    def __init__(self, sentence, embedding_size, transition_system):
        ...
        self.linear = nn.Linear(8 * embedding_size, 3 * embedding_size).to(device=constants.device)
    ...
    def subtree_rep(self, top, second, rel_embed,act_embed):
        ...
        reprs = torch.cat([top[0], second[0], rel_embed.reshape(self.embedding_size),act_embed.reshape(self.embedding_size)],
                          dim=-1)
    ...
    def right_arc_eager(self, act_emb):
        stack_top = self.stack[-1]
        buffer_first = self.buffer[0]
        self.stack.append(buffer_first)
        self.buffer.pop(0)
        self.arcs.append((stack_top[1], buffer_first[1]))
