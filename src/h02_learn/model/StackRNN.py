...
class NeuralTransitionParser(BaseParser):
    def __init__(self, vocabs, embedding_size, hidden_size, arc_size, label_size, batch_size,
        ...
        self.action_lstm = nn.LSTM(self.embedding_size, int(self.hidden_size / 2)).to(device=constants.device)
        ...
        self.empty_initial = nn.Parameter(torch.zeros(1, 1, int(self.hidden_size / 2)))
    ...
    def parse_step(self, parser, labeled_transitions, mode):
        # get parser state
        ...
        state = self.dropout(F.relu(self.mlp_lin2(parser_state)))
        ...
        if best_action == 0:
            # shift
            self.stack.push(self.buffer.pop(0))
            self.action.push(self.shift_embedding)
            parser.shift()
        elif best_action == 1:
            # reduce-l
            self.stack.pop(-1)
            self.action.push(self.reduce_l_embedding)
            ...
            self.action.push(self.reduce_r_embedding)
    ...
    def forward(self, x, transitions, relations, mode):
        ...
        # self.stack.push(self.gaurd)
        # self.buffer.push(self.gaurd)
        # self.action.push(self.gaurd_act)
