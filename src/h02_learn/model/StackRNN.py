...
class NeuralTransitionParser(BaseParser):
    def __init__(self, vocabs, embedding_size, hidden_size, arc_size, label_size, batch_size,
        ...
        self.mlp_lin2 = nn.Linear(self.embedding_size * 5, self.embedding_size * 4).to(device=constants.device)
        self.mlp_lin3 = nn.Linear(self.embedding_size * 4, 3*self.embedding_size).to(device=constants.device)
        ...
        self.mlp_lin1_rel = nn.Linear(self.embedding_size * 6 + 16,
                                      self.embedding_size * 5).to(device=constants.device)
        self.mlp_lin2_rel = nn.Linear(self.embedding_size * 5, self.embedding_size * 4).to(device=constants.device)
        self.mlp_lin3_rel = nn.Linear(self.embedding_size * 4, self.embedding_size*3).to(device=constants.device)
