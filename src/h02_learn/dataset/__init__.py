...
from h01_data import load_vocabs, load_embeddings, get_ud_fname, \
    get_oracle_actions  # ,get_oracle_actions_small,get_ud_fname_small
...
def generate_batch(batch):
    ...
    max_length = max([len(entry[1][0]) for entry in batch])
    ...
    return (text, text_mappings), (heads, rels), (transitions, relations_in_order)
...
def get_data_loader(fname, transitions_file, tokenizer, batch_size, shuffle):
    ...
    return DataLoader(dataset, batch_size=batch_size, shuffle=shuffle,
                      collate_fn=lambda batch: generate_batch(batch)), dataset.max_sent_len
...
def get_data_loaders(data_path, language, batch_size, batch_size_eval,
    ...
    trainloader, max_sent_len_train = get_data_loader(fname_train, transitions_train, tokenizer,
                                                      batch_size,
                                                      shuffle=True)
    devloader, max_sent_len_dev = get_data_loader(fname_dev, transitions_dev, tokenizer,
                                                  batch_size_eval,
                                                  shuffle=False)
    testloader, max_sent_len_test = get_data_loader(fname_test, transitions_test, tokenizer,
                                                    batch_size_eval,
                                                    shuffle=False)
