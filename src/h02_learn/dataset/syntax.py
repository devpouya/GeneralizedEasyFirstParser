...
class DerivationDataset(Dataset):
    def __init__(self, fname, oracle_file=None, hypergraph=None, tokenizer=None):
        self.fname = fname
        self.max_rel = 0
        self.model = hypergraph
        self.oracle_file = oracle_file
        self.tokenizer = tokenizer
        self.max_sent_len = 0
        self.load_data(fname, oracle_file)

        self.n_instances = len(self.words)

    def load_data(self, fname, oracle_file):
        self.words, self.word_st, self.pos, self.heads, self.rels = [], [], [], [], []
        self.actions = []
        self.mappings = []
        self.relations_in_order = []

        with open(fname, 'r') as file, open(oracle_file, 'r') as file2:
            for line, action in zip(file.readlines(), file2.readlines()):
                sentence = json.loads(line)
                hypergraph = json.loads(action)
                ...
                self.actions += [self.hypergraph2tensor(hypergraph)]
